...
function analyzeProject(project,source){
  ...
  const contractReports=contracts.map(contract=>{
    ...
    return {contractId,projectItemId,taskMatch,templateRefs,contactRefs};
  });
  return {
    source,
    projectId:projectIdOf(project),
    name:project.name ?? null,
    counts:{tasks:tasks.length,records:records.length,visible:records.filter(x=>!x.trashed&&!x.pendingDelete).length,trashed:records.filter(x=>x.trashed).length,done:records.filter(x=>x.done).length,contracts:contracts.length,templates:templates.length,contacts:contacts.length},
    contractReports,
...
function summarizeTemplate(t){
  return {id:idOf(t.id),title:t.title??t.name??null,trashed:t.trashed===true,hidden:t.hidden===true,deletedAt:t.deletedAt??null};
}
...
async function run(){
  ...
  ids.forEach(id=>{
    ...
    if(matches.length>1){
      const signatures=matches.map(a=>({source:a.source,counts:a.counts,refs:a.contractReports.map(r=>({contractId:r.contractId,projectItemId:r.projectItemId,templateRefs:r.templateRefs.map(t=>t.id)}))}));
  ...
  else setStatus('تشخیص کامل شد. گزارش را کپی کن و بفرست.','ok');
}
