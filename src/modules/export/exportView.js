...
<style>
  ...
  td.parent-num { font-family: 'IRANYekan', IRANYekan, Vazirmatn, Tahoma, sans-serif; font-size: 16px; font-weight: 700; color: #202124; text-align: center; width: 40px; }
  ...
  .row-cost { font-family: 'JetBrains Mono', monospace; font-size: 12.5px; font-weight: 600; color: #202124; direction: ltr; unicode-bidi: isolate; display: inline-flex; align-items: baseline; gap: 4px; }
  .row-cost .cost-unit { font-family: 'IRANYekan', IRANYekan, Vazirmatn, Tahoma, sans-serif; font-size: 10px; font-weight: 500; color: #5f6368; }
  ...
  tr.total-row td { border-bottom: none; border-top: 2px solid #202124; padding-top: 12px; font-weight: 700; }
  ...
  <table><thead><tr><th style="width:28px;"></th><th>مورد</th>${costHeader}</tr></thead><tbody>${rowsHtml}${totalRow}</tbody></table>
  ${noteHtml}${sigHtml}
  <script>window.onload = function(){ setTimeout(function(){ window.print(); }, 450); };<\/script>
<\/body><\/html>`;

  const w = window.open('', '_blank');
  if(!w){ toast('اجازهٔ باز شدن پنجره را بدهید'); return; }
  w.document.open();
...
async function generateProjectJpeg(){
  const p = findProject(exportPid);
  if(!p) return;
  if(!exportSelected.size){ toast('حداقل یک مورد را انتخاب کنید'); return; }
  const numCb = documentRef.getElementById('exportNumberedCb');
  const costCb = documentRef.getElementById('exportCostCb');
  if(numCb) exportMarkMode = numCb.checked ? 'number' : 'square';
  if(costCb) exportShowCost = costCb.checked;
