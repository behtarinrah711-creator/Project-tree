...
/** One-shot suppress is only valid briefly (same tick / child close). Stale flags must not block form Back. */
let suppressWorkspaceBackUntil = 0;
function markSuppressWorkspaceBack(){
  suppressWorkspaceBackOnce = true;
  suppressWorkspaceBackUntil = Date.now() + 120;
  try{ window.__karhaSuppressWorkspaceBackOnce = true; }catch(e){}
}
function shouldSuppressWorkspaceBack(){
  // Cross-module flag set by modular Search Template / numpad / jalali when hooks close via history.back().
  ...
  if(suppressWorkspaceBackOnce){
    if(Date.now() <= suppressWorkspaceBackUntil){
      return true;
    }
    // Stale one-shot (e.g. after child picker settled) — do not swallow form Back.
